	...
	*/
	...
	myapp.controller('Eventsctrl', ['$scope', '$http', '$routeParams', function ($scope, $http, $routeParams) {
		...
		$scope.myDateClick = function () {
			...
			var m = $scope.selectedDay.getMonth() + 1;
			var date = $scope.selectedDay.getDate() + '/' + m + '/' + $scope.selectedDay.getFullYear();
			console.log(date);
			$http({
				method: "POST",
			...
			$http({
				...
				for (var i = 0; $scope.days != undefined && i < $scope.days.length; i++) {
					...
					if ($scope.days[i] != day) {
						...
						if (document.getElementById('md-0-month-' + year + '-' + (month - 1) + '-' + $scope.days[i]) !== null) {
							document.getElementById('md-0-month-' + year + '-' + (month - 1) + '-' + $scope.days[i]).style.color = "rgb(0, 0, 0)";
							document.getElementById('md-0-month-' + year + '-' + (month - 1) + '-' + $scope.days[i]).style.backgroundColor = "rgb(240, 143, 10)";
	...
	myapp.controller('SeasonProjCtrl', ['$scope', '$http', '$filter', function ($scope, $http, $filter) {
		window.scrollTo(0, 0);
		...
		$http({
			...
			if (response.data.IsSuccess) {
				...
				$scope.f = response.data.Response;

				//$scope.SPro.push(response.data.Response[0]) ; 
				//		console.log($scope.activities) ; 
				console.log($scope.SPro);
			}
		});
		$scope.filtering = function (x) {
			$scope.SPro = $filter('filter')($scope.f, {
				Name: x
			});
			//console.log($scope.SPro) ; 
		}
